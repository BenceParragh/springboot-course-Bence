...
public class AirportControllerIT {
	...
	void testThatCreatedAirportIsListed() {
		...
		assertThat(airportsAfter.subList(0, airportsBefore.size())).usingRecursiveFieldByFieldElementComparator()
	...
	private void createAirport(AirportDto newAirport) {
		...
		webTestClient.post().uri(API_AIRPORTS).bodyValue(newAirport).exchange().expectStatus().isOk();

	}

	private List<AirportDto> getAllAirports() {
		List<AirportDto> allAirports = webTestClient.get().uri(API_AIRPORTS).exchange().expectStatus().isOk()
				.expectBodyList(AirportDto.class).returnResult().getResponseBody();
